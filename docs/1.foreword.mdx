...
import IntroTag, { TagType } from '@site/src/components/IntroTag';
...
1. <IntroTag type={TagType.must} />：严格按照规范要求的书写。
2. <IntroTag type={TagType.forbidden} />：绝对不能在本规范中出现。
3. <IntroTag type={TagType.should} />：推荐按照规范要求的去做，在特定的情况下可能存在特殊情况可以忽略，必须加注释说明。
4. <IntroTag type={TagType.shouldNot} />：在本规范中不应该出现，在特定的情况下可能存在特殊情况可以忽略，必须加注释说明。
5. <IntroTag type={TagType.optional} />：可选择的规范要求。
6. <IntroTag type={TagType.draft} />：暂未正式形成规范，还处于讨论阶段。
