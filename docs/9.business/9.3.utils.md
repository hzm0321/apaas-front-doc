...
const App: React.FC<Props> = () => {
  ...
  useEffect(() => {
    ...
    // 确定缓存标识
    const cacheKey = 'fetchData' + JSON.stringify(query);
    // 对普通请求使用 cachedRequest 包装
    // This will right
    const fetchDataCached = cachedRequest(fetchData, cacheKey);
    // 发起请求
    fetchDataCached(query).then((res) => {
      setData(res);
    });
  }, []);
